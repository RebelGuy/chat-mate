...
module.exports = (env) => {
  ...
  const isLocal = !nullOrEmpty(process.env.IS_LOCAL) ? Boolean(process.env.IS_LOCAL)
    : !nullOrEmpty(env.IS_LOCAL) ? Boolean(env.IS_LOCAL)
    : false
  const nodeEnv = !nullOrEmpty(env.NODE_ENV) ? env.NODE_ENV : 'debug'
