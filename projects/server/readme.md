...
## Local S3
ChatMate connects to a local S3 server called MinIO. Download it [here](https://min.io/download) and reference the path in the `MINIO_PATH` environment variable. The MinIO process starts automatically when running ChatMate locally. The data is saved in the `./data/minio` folder in the `chat-mate` root folder.

To get set up, it is advisable to run the server stand-alone via the `yarn workspace server start:minio` script. Navigate to the [WebUI](http://127.0.0.1:9000) and follow these steps:
- Log in with the default username `minioadmin` and password `minioadmin`
- Select Configuration in the sidebar, then set a region, noting it in the `S3_REGION` environment variable
- Create a new bucket, ensuring the name is consistent with the `S3_BUCKET` environment variable
- Create an access key and set the `S3_KEY` and `S3_SECRET` environment variables accordingly
- Set the `S3_DOMAIN` environment variable to `http://127.0.0.1:9000`

### Transferring from remote S3 to MinIO
- `sudo apt install awscli`
- `export AWS_ACCESS_KEY_ID=<key>`
- `export AWS_SECRET_ACCESS_KEY=<secret>`
- `aws s3 sync s3://chat-mate/ ~/temp --endpoint-url https://syd1.digitaloceanspaces.com`
- `aws s3 sync ~/temp s3://chat-mate/ --endpoint-url http://localhost:9000`
...
- `MINIO_PATH`: The local path to the MinIO executable. This is required to run the S3 server locally. Download it for your system [here](https://min.io/download).
...
## Custom errors
Rather than throwing generic Javascript `Error`s, we use `ChatMateError`s (or other errors deriving from these) to denote known/handled errors, which increases the robustness of the unit tests. Let's say a unit test is verifying that a call to a method is throwing. Traditionally, it would achieve this by checking that an error is thrown. However, how do we know the error didn't occur because of an unhandled exception, for example, accessing a property on a null object? If, instead, the test checks that an error of type `ChatMateError` was thrown, we can be much more certain that the logic is working as intended. It is always preferred to use more specific errors where possible (e.g. `ChatUserNotFoundError extends ChatMateError`), however, this is not always practical and can greatly pollute or complicate the code for little benefit.
