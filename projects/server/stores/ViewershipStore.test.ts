...
export default () => {
  ...
  /** Well outside the activeLivestream1 start/end boundaries */
  ...
  beforeEach(async () => {
    ...
    await db.livestream.createMany({ data: [
      ...
      { liveId: 'id3', streamerId: streamer1, continuationToken: null, start: data.time3, createdAt: data.time3, isActive: true, type: 'publicLivestream' },
      { liveId: 'id4', streamerId: streamer2, continuationToken: null, start: data.time2, createdAt: data.time2, isActive: true, type: 'publicLivestream' }
    ]})
    ...
    await db.viewingBlock.createMany({ data: [
      { userId: 3, livestreamId: 2, startTime: data.time2, lastUpdate: data.time2 },
      { userId: 3, livestreamId: 3, startTime: data.time3, lastUpdate: data.time3 },
    ]})
  ...
  describe(nameof(ViewershipStore, 'addLiveViewCount'), () => {
    test('correctly adds live viewer count', async () => {
      ...
      await viewershipStore.addLiveViewCount(activeLivestream1.id, youtubeViews, twitchViews)
  ...
  describe(nameof(ViewershipStore, 'addViewershipForChatParticipation'), () => {
    test('adds new viewing block if user not seen before', async () => {
      await viewershipStore.addViewershipForChatParticipation(activeLivestream1, user1, safeMsgTime3.getTime())
    ...
    test('trims viewing block to fit into livestream times', async () => {
      ...
      const livestream = { ...activeLivestream1, start, end }

      await viewershipStore.addViewershipForChatParticipation(livestream, user1, data.time3.getTime())
    ...
    test('extends previous viewing block if recent', async () => {
      ...
      await db.viewingBlock.create({ data: {
        livestream: { connect: { id: activeLivestream1.id }},
        user: { connect: { id: user1 }},
        startTime,
        lastUpdate: prevUpdate
      }})

      await viewershipStore.addViewershipForChatParticipation(activeLivestream1, user1, currentTime.getTime())
  ...
  describe(nameof(ViewershipStore, 'getLastSeen'), () => {
    test('returns null if never seen', async () => {
      // note: the test db doesn't contain viewing blocks for this user for this streamer
      await db.viewingBlock.create({ data: { livestreamId: activeLivestream2.id, userId: user1, startTime: new Date(), lastUpdate: new Date() }})

      const result = await viewershipStore.getLastSeen(streamer1, user1)
    ...
    test('returns time of correct viewing block', async () => {
      ...
      await db.viewingBlock.createMany({ data: [
        { userId: user1, livestreamId: inactiveLivestream1.id, startTime: time1, lastUpdate: time2 },
        { userId: user1, livestreamId: activeLivestream1.id, startTime: time3, lastUpdate: time4 },
        { userId: user1, livestreamId: activeLivestream1.id, startTime: time5, lastUpdate: time6 },
      ]})
  ...
  describe(nameof(ViewershipStore, 'getLatestLiveCount'), () => {
    test('returns null if no data exists for active livestream', async () => {
      ...
      const result = await viewershipStore.getLatestLiveCount(activeLivestream1.id)
    ...
    test('returns null if there is no active livestream', async () => {
      ...
      const result = await viewershipStore.getLatestLiveCount(inactiveLivestream1.id)
  ...
  describe(nameof(ViewershipStore, 'getLivestreamParticipation'), () => {
    ...
    test('returns ordered streams where users participated', async () => {
      ...
      // user2: livestream 2
      await db.chatMessage.createMany({ data: [
        { streamerId: streamer1, userId: user3, livestreamId: 3, time: data.time3, externalId: 'id3' },
      ]})
  ...
  describe(nameof(ViewershipStore, 'getLivestreamViewership'), () => {
    test('returns empty array if no viewership', async () => {
      await db.viewingBlock.createMany({ data: [
        { userId: user1, livestreamId: activeLivestream2.id, startTime: data.time1, lastUpdate: data.time1 }, // correct user, wrong streamer
    ...
    test('returns ordered streams where users participated', async () => {
      ...
      const time3 = data.time3
      await db.viewingBlock.createMany({ data: [
        { userId: user1, livestreamId: inactiveLivestream1.id, startTime: time1, lastUpdate: time1 }, // id 1
        { userId: user2, livestreamId: activeLivestream1.id, startTime: time2, lastUpdate: time2 }, // id 3
        { userId: user3, livestreamId: activeLivestream1.id, startTime: time2, lastUpdate: time2 }, // id 3, wrong user
        { userId: user2, livestreamId: activeLivestream2.id, startTime: time3, lastUpdate: time3 }, // id 4, wrong streamer
      ]})
