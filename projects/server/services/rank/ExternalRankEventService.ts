...
export default class ExternalRankEventService extends ContextClass {
  ...
  public async onTwitchChannelBanned (streamerId: number, channelName: string, moderatorChannelName: string, reason: string, endTime: number | null): Promise<void> {
    const data = await this.channelService.getTwitchDataForExternalRankEvent(streamerId, channelName, moderatorChannelName, 'punishment')
    if (data == null) {
  ...
  public async onTwitchChannelModded (streamerId: number, channelName: string): Promise<void> {
    const data = await this.channelService.getTwitchDataForExternalRankEvent(streamerId, channelName, null, 'administration')
    if (data == null) {
  ...
  public async onYoutubeChannelBanned (streamerId: number, channelName: string, moderatorChannelName: string) {
    const data = await this.channelService.getYoutubeDataForExternalRankEvent(streamerId, channelName, moderatorChannelName, 'punishment')
    if (data == null) {
