...
export type GetChatResponse = ApiResponse<6, {
  // include the timestamp so it can easily be used for the next request
  reusableTimestamp: number
  chat: Tagged<2, PublicChatItem>[]
}>

export type GetChatEndpoint = Endpoint<6, { since?: number, limit?: number }, GetChatResponse>
...
export default class ChatController extends ControllerBase {
  ...
  public async getChat (
    ...
    const builder = this.registerResponseBuilder<GetChatResponse>('GET /', 6)
    try {
